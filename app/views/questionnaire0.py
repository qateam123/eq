...
def get_thank_you(eq_id, form_type, collection_id):  # pylint: disable=unused-argument
    if request.method == "POST":
        ...
        return redirect(url_for('.get_feedback', eq_id=eq_id, form_type=form_type, collection_id=collection_id))
    else:
...
@questionnaire_blueprint.route('feedback', methods=["GET", "POST"])
@login_required
def get_feedback(eq_id, form_type, collection_id):
    if request.method == "POST":
        metadata = get_metadata(current_user)
        answer_store = get_answer_store(current_user)
        path_finder = PathFinder(g.schema_json, answer_store, metadata)
        ...
        group_id = SchemaHelper.get_last_group_id(g.schema_json)
        current_location = Location(group_id, group_instance=0, block_id='feedback')
        feedback_page = _build_template(current_location=current_location)
